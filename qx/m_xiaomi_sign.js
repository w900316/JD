...
!(async () => {
    ...
    if (!cookiesArr[0]) {
        ...
        authorCodeList = [
            '4a2a586de2034726b64408b46529b24a',
        ]
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            $.activityId = 'dz2111100000412301'
            $.activityShopId = '1000004123'
            $.activityUrl = `https://lzdz1-isv.isvjcloud.com/dingzhi/xiaomi/sign/activity?activityId=${$.activityId}&shareUuid=${encodeURIComponent($.authorCode)}&adsource=null&shareuserid4minipg=null&shopid=${$.activityShopId}&lng=00.000000&lat=00.000000&sid=&un_area=`
            await superFans();
...
async function superFans() {
    ...
    if ($.token) {
        ...
        if ($.secretPin) {
            ...
            await task('xiaomi/sign/activityContent', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=${encodeURIComponent($.pinImg)}&nick=${encodeURIComponent($.pin)}&cjyxPin=&cjhyPin=&shareUuid=${encodeURIComponent($.authorCode)}`)
            if ($.activityContent) {
                ...
                $.log("\n关注店铺")
                if (!$.activityContent['isFollowShop']) {
                    await task('xiaomi/sign/followShop', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&actorUuid=${$.actorUuid}&shareUuid=${encodeURIComponent($.authorCode)}&taskType=1&taskValue=1`)
                }
                if ($.activityContent['opencard']) {
                    $.log("\n去签到")
                    if (!$.activityContent['activeTask']['sign'].allStatus) {
                        await task('xiaomi/sign/saveTask', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&actorUuid=${$.actorUuid}&shareUuid=${encodeURIComponent($.authorCode)}&taskType=0&taskValue=1`)
                    } else {
                        $.log("签到过了\n")
                    }
                } else {
                        ...
                        if (data.result) {
                            switch (function_id) {
                                ...
                                case 'wxActionCommon/getUserInfo':
                                    ...
                                    break;
                                case 'xiaomi/sign/activityContent':
                                    if (!data.data.hasEnd) {
